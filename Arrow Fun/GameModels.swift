...
struct GameProgress: Codable {
    ...
    var bestScoresByLevel: [Int: Int] = [:]
    var totalScoresByLevel: [Int: Int] = [:]
    var levelSummaries: [LevelScoreSummary] = []

    enum CodingKeys: String, CodingKey {
        case highestUnlockedLevel
        case lastPlayedLevel
        case bestStarsByLevel
        case bestScoresByLevel
        case totalScoresByLevel
        case levelSummaries
    }

    init(
        highestUnlockedLevel: Int = 0,
        lastPlayedLevel: Int = 0,
        bestStarsByLevel: [Int: Int] = [:],
        bestScoresByLevel: [Int: Int] = [:],
        totalScoresByLevel: [Int: Int] = [:],
        levelSummaries: [LevelScoreSummary] = []
    ) {
        self.highestUnlockedLevel = highestUnlockedLevel
        self.lastPlayedLevel = lastPlayedLevel
        self.bestStarsByLevel = bestStarsByLevel
        self.bestScoresByLevel = bestScoresByLevel
        self.totalScoresByLevel = totalScoresByLevel
        self.levelSummaries = levelSummaries
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        highestUnlockedLevel = try container.decodeIfPresent(Int.self, forKey: .highestUnlockedLevel) ?? 0
        lastPlayedLevel = try container.decodeIfPresent(Int.self, forKey: .lastPlayedLevel) ?? 0
        bestStarsByLevel = try container.decodeIfPresent([Int: Int].self, forKey: .bestStarsByLevel) ?? [:]
        bestScoresByLevel = try container.decodeIfPresent([Int: Int].self, forKey: .bestScoresByLevel) ?? [:]
        totalScoresByLevel = try container.decodeIfPresent([Int: Int].self, forKey: .totalScoresByLevel) ?? [:]
        levelSummaries = try container.decodeIfPresent([LevelScoreSummary].self, forKey: .levelSummaries) ?? []
    }

    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(highestUnlockedLevel, forKey: .highestUnlockedLevel)
        try container.encode(lastPlayedLevel, forKey: .lastPlayedLevel)
        try container.encode(bestStarsByLevel, forKey: .bestStarsByLevel)
        try container.encode(bestScoresByLevel, forKey: .bestScoresByLevel)
        try container.encode(totalScoresByLevel, forKey: .totalScoresByLevel)
        try container.encode(levelSummaries, forKey: .levelSummaries)
    }
}
...
struct LevelScoreSummary: Codable, Hashable, Identifiable {
    let id: Int
    var levelIndex: Int
    var levelNumber: Int
    ...
    var lastScore: Int
    var lastStars: Int
    var lastMovesUsed: Int
    var lastMoveLimit: Int
    var lastElapsedSeconds: TimeInterval
    var playCount: Int
    var lastCompletedAt: Date
}

struct ArrowWeaveRoute: Identifiable, Hashable {
    let id: String
    let path: [BoardPoint]
    ...
    var endPoint: BoardPoint {
        path.last ?? startPoint
    }
    ...
    var direction: Direction {
        ...
        let delta = BoardPoint(row: last.row - previous.row, col: last.col - previous.col)
        switch delta {
...
struct ArrowWeaveLevelDefinition: Identifiable, Hashable {
    let id: String
    let number: Int
    let title: String
    let subtitle: String
    ...
    let cols: Int
    let moveLimit: Int
    let routes: [ArrowWeaveRoute]
    let mechanicNotes: [String]

    var totalRoutes: Int { routes.count }
}
