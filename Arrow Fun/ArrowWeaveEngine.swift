...
enum ArrowWeaveLevelFactory {
    ...
    static func makeLevel(number: Int) -> ArrowWeaveLevelDefinition {
        ...
        let moveLimit = routeCount + 2 + min(stage / 6, 3)
...
final class ArrowWeaveSession: ObservableObject {
    ...
    var score: Int {
        let routeBonus = escapedRoutes * 140
        let moveBonus = remainingMoves * 90
        let flowBonus = max(0, level.totalRoutes - movesUsed) * 25
        let completionBonus = completed ? 320 : 0
        return max(0, routeBonus + moveBonus + flowBonus + completionBonus)
    }
