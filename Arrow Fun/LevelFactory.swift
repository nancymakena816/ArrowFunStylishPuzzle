...
enum LevelFactory {
    ...
    private static func campaignLevel(number: Int, stage: Int) -> LevelDefinition {
        ...
        let resolvedMoveLimit = max(moveLimit, arrowCount)
    ...
    private static func buildLevel(
        ...
        let resolvedMoveLimit = max(moveLimit, arrows.count)
    ...
    private static func safeFallbackLevel(number: Int, world: Int, stage: Int) -> LevelDefinition {
        ...
        var secondRow = min(rows - 2, firstRow + 2)
        if secondRow == firstRow {
            secondRow = max(1, firstRow - 2)
        }
        if secondRow == firstRow {
            secondRow = rows > 3 ? 1 : max(0, rows - 2)
        }
        ...
        let resolvedMoveLimit = max(6 + arrowCount + (stage / 10), arrows.count)
